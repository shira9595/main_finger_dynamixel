    ...
    def connect(self):
        try:
            if self.port_handler.openPort():
                time.sleep(2) ##think about this time
...
class Arduino_Communication:
    def __init__(self, port, baud_rate=9600):
        self.port = "COM4" ##### i need to make sure
...
def main():
    arduino = Arduino_Communication(Arduino_Communication.port)
    dynamixel.port_handler.openPort()
    dynamixel.port_handler.setBaudRate(dynamixel.baudrate)
    time.sleep(0.5)
    while True:
        try:
